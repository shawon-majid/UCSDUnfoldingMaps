...
public class EarthquakeCityMap extends PApplet {
	...
	private void addKey() {	
		...
		fill(0, 0, 0);
		text("City Marker", 65, 100);
		text("Land Quake", 65, 120);
		text("Ocean Quake", 65, 140);
		text("Size ~ Magnitude", 45, 155);
		
		fill(color(255, 255, 0));
		ellipse(50, 200, 12, 12);
		fill(color(0,0, 255));
		ellipse(50, 220, 12, 12);
		fill(color(255, 0,0));
		ellipse(50, 240, 12, 12);
		
		fill(0,0,0);
		text("Shallow", 65, 200);
		text("Intermediate", 65, 220);
		text("Deep", 65, 240);
		...
		for(Marker cm: countryMarkers) {
			...
			for(Marker m: quakeMarkers) {
				...
				if(em.isOnLand()) {
					...
					if(countOcean)oceanCounter++;
				}
			}
			
			
			if(quakeCounter >= 1) System.out.println(countryName + " : " + quakeCounter);
			countOcean = false;
