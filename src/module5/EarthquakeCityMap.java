...
public class EarthquakeCityMap extends PApplet {
	...
	public void mouseMoved()
	{
		...
		if (lastSelected != null) {
			lastSelected.setSelected(false); // it comes from the AbstractMarker Class by simple point marker to common marker
			lastSelected = null;
	...
	public void mouseClicked()
	{
		...
		if (lastClicked != null) {
			...
			clickQuakeMarker(quakeMarkers);
			clickCityMarker(cityMarkers);
			
		}
		
	}
	
	private void clickQuakeMarker(List<Marker> markers) {
			...
			if(!em.isHidden() && em.isInside(map, mouseX, mouseY)) {
				...
				for(Marker cMarker: cityMarkers) {
					if(cMarker != lastClicked) {
						if(cMarker.getDistanceTo(lastClicked.getLocation()) > em.threatCircle()) {
	...
	private void clickCityMarker(List <Marker> markers) {
		for(Marker m: markers) {
			if(!m.isHidden() && m.isInside(map, mouseX, mouseY)) {
				...
				// hide all earthquake whose threadCircle do not cover m
				
				for(Marker eMarker: quakeMarkers) {
					EarthquakeMarker em = (EarthquakeMarker)eMarker;
					if(em.threatCircle() < m.getDistanceTo(em.getLocation())) {
						em.setHidden(true);
					}
				}
				return;
